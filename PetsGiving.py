...
def send_pet(cmd_src: CommandSource, pet_in: str, pet_name: str, target :str): 
    ...
    if target_match is not None:
        ...
        return
    command  = None
    if pet in PetCategory.TAME_OWNER:
    ...
    elif pet in PetCategory.OWNER:
        command = 'execute at %s '%cmd_src.player
        +'as @e[type=minecraft:%s,distance=..10,name=\"%s\",nbt={Owner:%s}] '%(pet, pet_name, sourceUUID)
        +'run data modify entity @s Owner set value %s'%targetUUID
    elif pet in PetCategory.TRUSTED:
        tag = str(int(time.time()))
        mark = 'PetsGiving'
        server = plugin_fields.server
        server.rcon_query(
            'execute as @e[type=minecraft:%s,distance=..10,name=\"%s\"] '%(pet, pet_name)
        ...
        while(server.rcon_query('execute if entity @e[tag=%s]'%tag) != 'Test failed'):
            ...
            if trusted_match is not None:
                ...
                trustedUUID_2 = trusted_match.group(2)
            else:
            ...
            # ^ replace UUID
            if UUID2 is None:
                set_command = 'data modify entity @e[tag=%s,tag=%s] Trusted set value [%s]'%(tag,mark,UUID1)
            else:
                set_command = 'data modify entity @e[tag=%s,tag=%s] Trusted set value [%s,%s]'%(tag,mark,UUID1,UUID2)
...
def reg_commands():
    ...
    server.register_command(
        ...
        then(
            Text('pet_in').
            requires(lambda src,ctx: ctx['pet_in'] in PETS).
            then(
                Text('pet_name').
                then(
                    Literal('to').
    ...
    server.register_command(
        ...
        then(
            ...
            then(
                Text('pet_name').
                runs(lambda src,ctx: send_pet(src,ctx['pet_in'],ctx['pet_name']))                
            )
        )
    )
